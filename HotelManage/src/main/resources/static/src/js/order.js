var order = {
    ...
    funcs: {
        renderTable: function () {
            ...
            var addBtn = $("#add");
            order.funcs.bindAddEventListener(addBtn);
            ...
            addBtn.on('click', function () {
                ...
                layer.open({
                    ...
                    content: "<div id='addModal'>" +
                        ...
                        "<p style='padding: 5px 0px 5px 0px;'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;姓名:<input type='text' id='name'/></p>" +
                        "<p style='padding: 5px 0px 5px 0px;'>身份证号:<input type='text' id='id'/></p>" +
                        "<p style='padding: 5px 0px 5px 0px;'>联系方式:<input type='text' id='phone'/></p>" +
                        "<p style='padding: 5px 0px 5px 0px;'>入住时间:<input type='datetime-local' id='starttime'/></p>" +
                        "<p style='padding: 5px 0px 5px 0px;'>离开时间:<input type='datetime-local' id='endtime'/></p>" +
                        "<p style='padding: 5px 0px 5px 0px;'>是否会员:<select id='ismenber'>" +
                        home.funcs.generateSelect(2, home.vars.member) +
                        "</select></p>" +
                        "<p style='padding: 5px 0px 5px 0px;'>是否入住:<select id='isenter'>" +
                        home.funcs.generateSelect(2, home.vars.enter) +
                        "</select></p>" +
                        "<p style='padding: 5px 0px 5px 0px;'>房间数量:<input type='number' id='roomcount'/></p>" +
                        "<p style='padding: 5px 0px 5px 0px;'>客房编号:<input type='text' id='roomnos' placeholder='多个房间请用|分隔'/></p>" +
                        "<p style='padding: 5px 0px 5px 0px;'>订单价格:<input type='number' id='price'/></p>" +
                        "</div>" +
                        "</div>",
                    area: ['350px', '500px'],
                    btn: ['确认', '取消'],
                    ...
                    yes: function (index) {
                        ...
                        var id = $('#id').val()
                        var phone = $('#phone').val()
                        var starttime = home.funcs.timeStr($('#starttime').val())
                        var endtime = home.funcs.timeStr($('#endtime').val())
                        var ismenber = $('#ismenber').val()
                        var isenter = $('#isenter').val()
                        var roomcount = $('#roomcount').val()
                        var roomnos = $('#roomnos').val()
                        var price = $('#price').val()
                        $.post(home.urls.order.insert, {
                            name: name,
                            id: id,
                            phone: phone,
                            starttime: starttime,
